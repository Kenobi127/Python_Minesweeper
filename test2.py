    ...
    def on_left_click(self, cur_button, y, x):
            ...
            for y in range(self.rows):
                for x in range(self.cols):
                    ...
                    if self.solution_board[y][x] == "x":
                        self.buttons[y][x].config(text=f"{self.solution_board[y][x]}", bg = "#FF4433", state=tk.DISABLED, relief=tk.SUNKEN)
                    else:
                        self.buttons[y][x].config(state=tk.DISABLED)
            self.game_over = True
            self.title["fg"] = "#FF4433"
        ...
        if self.empty_squares == 0:                 #no empty squares left, WIN
            for y in range(self.rows):
                for x in range(self.cols):
                    if self.solution_board[y][x] == "x":
                        self.buttons[y][x].config(text=f"{self.solution_board[y][x]}", bg = "#AFE1AF", state=tk.DISABLED)
            self.game_over = True
            self.title["fg"] = "#AFE1AF"
